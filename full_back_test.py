...
def showNextIndicatorData(ticker, frame, ema, vwap):
    ...
    print(
        Fore.RED + f"{vwap} || SELL || Ticker: {ticker}, Frame: {frame}, Profit number: {dataSell['profitNum']}, Lose number: {dataSell['loseNum']} || ALL WIN CHANGE: +{total_win_pct}% || ALL LOSE CHANGE: {total_lose_pct}% || SUCCESS PCT: {round((dataSell['profitNum'] / (dataSell['profitNum'] + dataSell['loseNum'])) * 100, 2)}%")
    ...
    print(
        Fore.GREEN + f"{vwap} || BUY || Ticker: {ticker}, Frame: {frame}, Profit number: {dataLong['profitNum']}, Lose number: {dataLong['loseNum']} || ALL WIN CHANGE: +{total_win_pct}% || ALL LOSE CHANGE: {total_lose_pct}% || SUCCESS PCT: {round((dataLong['profitNum'] / (dataLong['profitNum'] + dataLong['loseNum'])) * 100, 2)}%\n")
...
def nweIndicatorBackText():
    ...
    for frame in ["4h"]:
        ...
        th = threading.Thread(target=boost,
                              args=(showNextIndicatorData, const_app.tickers, frame, '', ''))
